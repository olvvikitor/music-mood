...
export function InsightsSection() {
    ...
    return (
        <div
            ...
            style={{
                background: "linear-gradient(135deg, rgba(176,106,133,0.06) 0%, rgba(12,12,16,0.70) 60%)",
            }}
        >
            {/* Glow */}
            <div className="absolute -top-8 -right-8 w-40 h-40 rounded-full blur-[60px] opacity-20 pointer-events-none"
                style={{ background: "radial-gradient(circle, #b06a85, transparent)" }} />

            {/* Label */}
            <div className="flex items-center gap-2 mb-2.5 md:mb-3 relative z-10">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0"
                    style={{ background: "rgba(176,106,133,0.15)", border: "1px solid rgba(176,106,133,0.3)" }}>
                    <Sparkles className="w-3 h-3" style={{ color: "#b06a85" }} />
                </div>
                <span className="text-[10px] uppercase tracking-[0.18em] font-800"
                    style={{ color: "#b06a85", fontFamily: "var(--font-display)", fontWeight: 800 }}>
                    Insight do dia
                </span>
            </div>
